  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(data.user.role === 'admin' ? '/admin' : 
         data.user.subscription.status === 'active' ? '/dashboard' : '/subscribe');
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ color: 'var(--cream-dim)', marginTop: '0.5rem' }}>Sign in to your account</p>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', color: 'var(--cream-dim)', fontSize: '0.9rem' }}>
          No account?{' '}
          <Link to="/register" style={{ color: 'var(--gold)', transition: 'opacity 0.2s' }}>
            Join GolfGives
          </Link>
        </p>
      </div>
